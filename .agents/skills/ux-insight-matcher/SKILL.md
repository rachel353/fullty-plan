...
.Codex/rules/design_system_rules.mdc  # If exists
```
...
Read all JSON files from `.Codex/design-lib/youtube/insights/`:

```
.Codex/design-lib/youtube/insights/ux_insights_*.json
```
...
```
.Codex/design-lib/youtube/insights/           # UX insight JSON files
aging-lab/app/globals.css      # Design tokens (PRIORITY 1)
.Codex/rules/design_system_rules.mdc  # Design rules (if exists)
```
