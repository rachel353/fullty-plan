  ...
  return (
    <div className="space-y-6">
      <div className="border-b border-border pb-4 flex items-end justify-between">
        ...
        <Link href="/get/new"><Button size="sm">새 요청 작성</Button></Link>
      </div>

      <div className="space-y-3">
        {getRequests.map((req) => {
          const reqProposals = proposals.filter((p) => p.getRequestId === req.id);
          return (
              ...
              {reqProposals.length > 0 && (
                <div className="mt-4 pt-4 border-t border-border">
                  <div className="text-xs font-semibold mb-3">
                    셀러 제안 {reqProposals.length}건 (풀티 검수 완료)
                  </div>
                      ...
                      <div key={p.id} className="border border-border p-3 flex items-center justify-between">
                        <div>
                          <div className="text-[11px] text-muted-foreground">
                            {p.productBrand} · 등급 {p.productGrade}
                          </div>
                        ...
                        <div className="flex gap-2">
                          <Link href={`/mypage/get/proposals/${p.id}`}>
                            <Button size="sm" variant="outline">상세</Button>
                          </Link>
                          <Link href={`/mypage/get/proposals/${p.id}/pay`}>
