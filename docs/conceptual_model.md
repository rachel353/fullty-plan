...
| profileImageUrl | String | X | 프로필 이미지 URL (미설정 시 기본 아바타) |
| creatorBio | String | X | 작가 소개글 (작가 전용, 500~1000자, Plain Text) |
| role | Enum | O | 사용자 역할 |
| authProvider | Enum | O | 인증 방식 |
| status | Enum | O | 계정 상태 |
| isVerified | Boolean | O | 본인인증 완료 여부 |
| phone | String | X | 휴대폰 번호 (회원가입 시 인증용, change-2026-01-14-1500) |
| isPhoneVerified | Boolean | O | 휴대폰 인증 완료 여부 (change-2026-01-14-1500) |
| consents | JSON | O | 약관 동의 상태 {serviceTerms, privacyPolicy, communityRules, marketingConsent} |
...
1. 이메일/비밀번호 입력 (비밀번호 규칙: 8자 이상, 영문, 숫자, 특수문자(선택))
2. 비밀번호 확인
3. 휴대폰 번호 입력 + 인증번호 전송
4. 인증번호 입력 (유효시간 5분)
5. 회원가입 완료 → 프로필 설정 페이지 이동
...
**비밀번호 재설정 단계:**
1. 로그인 화면 → 비밀번호 찾기 버튼 클릭
2. 휴대폰 번호 입력 + 인증번호 전송
3. 인증번호 입력 및 검증
4. 새 비밀번호 / 비밀번호 확인 입력 (동일 규칙 적용)
5. 재설정 완료 → 로그인 페이지 이동
